...
def get_tiny_images(image_paths):
    ...
    dimension = 9
    #dimension表示这里将所有的图像都resize为一个9*9的小图像
    #构造输出的矩阵n*d
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i in range(len(image_paths)):
        ...
        location,sift_features = sift_model.detectAndCompute(image_gray,None)
        #location表示的是sift特征点的数目，sift_features是一个矩阵：X*128
        for j in sift_features:
            SIFT_features.append(j)
    SIFT_features = np.array(SIFT_features)
    kmeans_model = KMeans(n_clusters=vocab_size,max_iter=300).fit(SIFT_features)
    #使用Kmeans得到的聚类的簇的中心为词袋模型中的词
...
def get_bags_of_words(image_paths):
    ...
    for i in range(len(image_paths)):
        ...
        image_gray = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
        location,sift_features = sift_model.detectAndCompute(image_gray,None)
        #得到每个图片的sift特征,下面计算每个图像的hist直方图统计距离选择最近的一个计数+1
        for j in range(len(location)):
            distance = cdist(np.array([sift_features[j]]),vocab,metric='euclidean')
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # TODO: Implement this function!
    #训练SVM分类器
    SVM_classification = LinearSVC(penalty='l2',multi_class='ovr',random_state=0,max_iter=100)
